...
import { Users, Plus, Trash2, Shield, User, Sparkles, Check, Mail, Lock } from 'lucide-react';
...
export const CollaboratorsTab: React.FC<CollaboratorsTabProps> = ({
  ...
  const { canAssignDevelopers } = useAuth();
  ...
  const handleAddDeveloper = () => {
    if (!canAssignDevelopers || !selectedUserId) return;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs">
        ...
        <p className="text-xs text-slate-500 mt-1 max-w-2xl leading-relaxed">
          {canAssignDevelopers
            ? 'Assign faculty developers to this workshop from the directory. Assigned developers can collaborate on building outline modules and activities.'
            : 'Multi-author team for this workshop. Only Workshop Leads and Program Administrators can assign or reassign developers.'}
        </p>
      </div>

      {/* Add Developer Bar - Only for Workshop Leads & Admins */}
      {canAssignDevelopers ? (
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-xs text-slate-600 flex items-center gap-2">
          ...
          <span>Assigning developers is reserved for Workshop Leads and Program Administrators.</span>
        </div>
      )}
